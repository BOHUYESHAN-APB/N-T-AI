...
class _CharacterDisplayState extends State<CharacterDisplay> {
  ...
  void initState() {
    ...
    if (widget.expressionAgent != null) {
      _expressionSub = widget.expressionAgent!.stream.listen(_onExpressionUpdate);
      _motionSub = widget.expressionAgent!.motionStream.listen(_onMotionRequest);
    }
  }
  ...
  Future<String> _getModelUrl() async {
    ...
    // Enable debug mode to see on-screen logs in the WebView
    return '${widget.backendUrl}/static/live2d/index.html?model=$path&debug=true';
  }
  ...
  void _onExpressionUpdate(ExpressionData data) {
    ...
    final js = "if (window.live2dManager) window.live2dManager.updateParameters($jsonStr);";
  ...
  void _onMotionRequest(MotionRequest req) {
    ...
    final js = "if (window.LanLan1 && window.LanLan1.askMotionAgent) window.LanLan1.askMotionAgent($u, $a);";
