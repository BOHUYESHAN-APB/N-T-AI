...
export async function getServer(host, port, version) {
    ...
    // Server not found
    if (server == null) {
        console.warn(`MC server not found. (Host: ${host}, Port: ${port}) Check the host and port in settings.js, and ensure the server is running and open to public or LAN.`);
        return { host, port, version: version === "auto" ? "1.21.1" : version };
    }
    ...
    const isSupported = mc.supportedVersions.some(v => 
        serverVersion === v || (serverVersion.startsWith(v) && serverVersion.charAt(v.length) === '.') || (v.startsWith(serverVersion) && v.charAt(serverVersion.length) === '.')
