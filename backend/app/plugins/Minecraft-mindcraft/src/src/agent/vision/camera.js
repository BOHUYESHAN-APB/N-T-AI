...
let createCanvas;
try {
    const canvasModule = await import('node-canvas-webgl/lib/index.js');
    createCanvas = canvasModule.createCanvas;
} catch (e) {
    console.warn('Warning: node-canvas-webgl not found. Camera functionality will be disabled.');
}
...
export class Camera extends EventEmitter {
    constructor (bot, fp) {
        ...
        if (createCanvas) {
            ...
            console.error('Camera cannot be initialized: createCanvas is not available.');
        }
    }
    ...
    async capture() {
        if (!this.canvas) {
            console.error('Capture failed: Canvas not initialized.');
            return null;
        }
