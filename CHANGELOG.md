...
## [0.3.16-beta] - 2026-01-02

### 新增 (Added)
- **Linux 子系统配置**：新增 `/api/linux/config` 读写接口，前端状态弹窗可调整插件自启/容器自启/按需启动/禁用 WSL/下载链接等配置。
- **屏幕感知注入节奏**：新增“注入间隔/空闲注入阈值”配置，支持按 30/60 秒节奏注入。

### 优化 (Changed)
- **屏幕感知队列**：截图分析结果先排队汇总，避免生成中插入；必要时合并为简短摘要后再注入。
- **视觉路由策略**：屏幕截图优先走主模型视觉，缺失时再使用独立视觉服务商。
- **Linux 启动策略**：容器默认不随启动拉起，AI 执行命令时按需启动（可配置）。

### 修复 (Fixed)
- **Flutter 编译诊断**：清理多处无效类型检查/未使用方法提示，保持构建输出干净。

<!-- LATEST_LOG_SPLIT: 最新日志与历史日志分割线，请勿删除，这个分割线使用与CL自动截取最新分割线的，每次变更后需要自动转移到最新的部分放置，防止CL出错 -->
---
